...
class Makeup_fragment : Fragment() {
        ...
        inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View?
    {
        ...
        recyclerView = view.findViewById(R.id.rv_makeup)
        recyclerView.layoutManager = GridLayoutManager(activity, 2)
        recyclerView.setHasFixedSize(true)

        top_photo = view.findViewById(R.id.image_top_makeup)
    ...
    private fun addTopPhoto()
    {
        ...
        database.addValueEventListener(object: ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if(snapshot.exists())
                {
                    val photo = snapshot.child("makeup").value.toString()
    ...
    {
        database = FirebaseDatabase.getInstance(base_url).getReference("makeup")
        database.addValueEventListener(object: ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if(snapshot.exists())
                {

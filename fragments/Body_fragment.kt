...
class Body_fragment : Fragment() {
        ...
        inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View?
    {
        ...
        top_photo = view.findViewById(R.id.image_top_body)
    ...
    private fun addTopPhoto()
    {
        ...
        database.addValueEventListener(object: ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if(snapshot.exists())
                {
                    val photo = snapshot.child("body").value.toString()
    ...
    {
        database = FirebaseDatabase.getInstance(base_url).getReference("body")
        database.addValueEventListener(object: ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if(snapshot.exists())
                {
